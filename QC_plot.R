...
boxplot <- ggplot(qc, aes(x = position, y=score, weight=count, group=position), stat='identity')+
  geom_boxplot(fill="white", colour="cornflowerblue",
               outlier.alpha = 0.01, outlier.colour="tomato", outlier.size=1.0)+
  scale_x_continuous(expand=c(0,0), breaks=seq(0,nrow(xyz),by=5)) +
  scale_y_continuous(expand=c(0,0), breaks=seq(0,40, by=2)) +
  theme(axis.text.x=element_text(colour="cornflowerblue", size=12),
        axis.text.y=element_text(colour="cornflowerblue", size=12),
        axis.title=element_text(size=18)) +
  xlab("Read Position") +
  ylab("Quality Scores")
ggsave(filename="QC_plot.tiff", plot=boxplot, width=15, height= 15, dpi=600, compression = "lzw")
gc
