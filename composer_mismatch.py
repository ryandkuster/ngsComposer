...
def comp_main():
    ...
    composer(input1, input2, output1, output2, mismatch, chunk, barcodes, project_dir, True)
...
def comp_piper(input1_list, input2_list, mismatch, R1_barcodes, project_dir, input1):
    ...
    output1 = os.path.basename(input1)
    output2 = os.path.basename(input2)
    composer(input1, input2, output1, output2, mismatch, 3000000, R1_barcodes, project_dir, True)

def composer(input1, input2, output1, output2, mismatch, chunk, barcodes, project_dir, round_one):
    ...
    if round_one == True:
        ...
        # consider removing the unknowns file
        sys.exit()
...
def mismatcher(output1, output2, mismatch, chunk, barcodes, project_dir):
    input1 = project_dir + '/' + str(output_data1[0]) + '_' + output1
    input2 = project_dir + '/' + str(output_data1[0]) + '_' + output2
    composer(input1, input2, output1, output2, mismatch, chunk, barcodes, project_dir, False)
